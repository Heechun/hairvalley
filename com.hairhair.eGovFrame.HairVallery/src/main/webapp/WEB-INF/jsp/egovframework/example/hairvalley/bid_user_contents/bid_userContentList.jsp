				...
				<div class="row margin-vert-40">
					<div
						...
						<div class="panel-heading">
							<h3 class="panel-title">내 전체 입찰목록 조회</h3>
						</div>
								...
								<div class="col-md-12">
									...
									<div class="row margin-vert-10">
										<div class="col-md-8">
											<ul class="pagination">
												<c:if test="${page > 1}">
													<li><a href="bid_selectUserBidContent.do?user_id=${user_id}&page=${page-1}">&laquo;</a>
													</li>
												</c:if>
												<c:if test="${page == 1}">
												...
												<c:forEach var="num" begin="1" end="${totalpage}" step="1">
													<c:if test="${page == num}">
														<li class="active"><a
															href="bid_selectUserBidContent.do?user_id=${user_id}&page=${num}">${num}</a></li>
													</c:if>
													<c:if test="${page != num}">
														<li><a href="bid_selectUserBidContent.do?user_id=${user_id}&page=${num}">${num}</a>
														</li>
													</c:if>
												</c:forEach>
												<li><c:if test="${fn:length( bidBoardList ) < 10}">
														...
														<li><a href="bid_selectUserBidContent.do?user_id=${user_id}&page=${page+1}">&raquo;</a>
														</li>
													</c:if></li>
											</ul>
										</div>
